...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    exclude-result-prefixes="xs" xmlns="http://www.tei-c.org/ns/1.0" xmlns:mets="http://www.loc.gov/METS/"
    ...
    <xsl:template match="/">
        <xsl:result-document href="teifrommets.xml"
            exclude-result-prefixes="xsi pc mets">
            <TEI>
                <teiHeader>
                    <fileDesc>
                        <titleStmt>
                        ...
                        <xsl:element name="surfaceGrp">
                            ...
                            <xsl:element name="surface">
                                <xsl:element name="graphic">
                                    ...
                                    <xsl:attribute name="height"><xsl:value-of select="concat(document($filename)//pc:Page/@imageHeight,'px')"/></xsl:attribute>
                                </xsl:element>
                            ...
                            <xsl:element name="div">
                                ...
                                <xsl:for-each select="mets:file">
                                    <xsl:element name="p">
                                        ...
                                        <xsl:variable name="filename">
                                            <xsl:value-of
                                                select="substring-after(mets:FLocat/@href, '/./')"/>
                                        </xsl:variable>
                                        <xsl:for-each
                                            select="document($filename)//pc:TextLine//pc:Unicode">
                                            <xsl:element name="l">
                                                <xsl:attribute name="ref">
